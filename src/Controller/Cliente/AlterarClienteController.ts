...
class AlterarClienteController{
   async handle(req:Request, res:Response){
    ...
    const {id,nome,cpf_cnpj,rg_Ie,celular,fixo,rua,complemento,cep,bairro,cidade,estado } = req.body

    const alterarClienteController = new AlterarClienteServer()
     const alterarCliente = await  alterarClienteController.execute({
        id,nome,cpf_cnpj,rg_Ie,celular,fixo,rua,complemento,cep,bairro,cidade,estado
     } ) 
